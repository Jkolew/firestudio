...
const ACTIONS={
  hug:    ['안았','안겨','안아줬','껴안','포옹'],
  kiss:   ['키스','입맞','뽀뽀'],
  cry:    ['울었','눈물','슬퍼서','펑펑','울다','흘렸','흑흑','훌쩍'],
  laugh:  ['웃었','웃음','웃었다','크게 웃','하하','깔깔','재밌','신나','즐거'],
  write:  ['일기를','일기 썼','일기 씀','적었다','써봤','기록했','메모했','노트에','노트를 씀'],
  read:   ['읽었다','독서했','책 읽','읽다가','읽으며','읽고 있','공부'],
  drink:  ['마셨다','커피를 마','음료를 마','한 모금','홀짝','마시며'],
  think:  ['생각했','고민했','떠올렸','상상했','생각해봤','고민해봤','머릿속'],
  stretch:['기지개','스트레칭','일어났다','깼다','눈을 떴'],
  game:   ['게임','롤','배그','피씨방','pc방','게임 켰','피방','발로','옵치','승리','패배','게이밍'],
  eat:    ['먹었','밥 먹','식사했','점심 먹','저녁 먹','아침 먹','라면','피자','치킨','초밥','국밥','맛집'],
  sleep:  ['잤다','잠들','잠이','졸렸','누웠','낮잠','숙면','꿈나라'],
  walk:   ['걸었','산책했','걷다가','나갔다','나왔다','걸어갔','등교','하교','출근','퇴근'],
  run:    ['뛰었','달렸','뛰어갔','달려갔','조깅','질주'],
  work:   ['공부했','일했','작업했','공부 중','숙제했','코딩했','열일','발표','프로젝트'],
  talk:   ['말했','얘기했','대화했','이야기했','통화했','물었다','수다','상담'],
  wave:   ['인사했','손 흔들','손을 흔들','안녕'],
  dance:  ['춤췄','댄스했','춤을 췄','흥얼','둠칫'],
  look:   ['봤다','바라봤','쳐다봤','올려다봤','드라마를','영화를','티비를','넷플','유튜브'],
  sit:    ['앉았다','앉아서','앉아있','앉아 있','대기','멍때'],
  jump:   ['뛰어올랐','점프했','점프를'],
};
// ═══ PROP DETECTION ═══
const PROP_DETECT={
  phone:    ['핸드폰','스마트폰','폰을','폰이','폰으로','폰 보','카톡','문자를','셀카','사진 찍','인스타','디엠'],
  game:     ['게임','롤','배그','피씨방','pc방','플스','닌텐도','스팀','마크','오버워치','발로','피방'],
  computer: ['컴퓨터','노트북','피씨','작업 중','코딩','맥북','아이맥'],
  book:     ['책을','책이','독서','읽었','교과서','노트를','필기했','공부했','시험','도서관'],
  coffee:   ['커피','아메리카노','라떼','카페라','아이스티','따뜻한 음료','카페','스벅','투썸'],
  food:     ['밥을','밥이','음식','먹었','점심','저녁','아침','라면','피자','치킨','초밥','국밥','고기','파스타'],
  music:    ['음악','노래를','이어폰','헤드폰','듣고','플레이리스트','멜론','스포티파이','에어팟','버즈'],
  tv:       ['티비','tv를','드라마','영화를','유튜브','넷플릭스','애니','왓챠','디즈니'],
...
// ═══ LOCATION DETECTION ═══
const LOCATIONS={
  home:   ['집','방','침대','소파','거실','베란다','내 방'],
  school: ['학교','교실','도서관','캠퍼스','수업','강의','학원'],
  outside:['밖','공원','길','거리','야외','산책로','운동장','현관'],
  cafe:   ['카페','커피숍','스타벅스','이디야','투썸','투썸플레이스'],
  work:   ['회사','사무실','직장','업무'],
...
function startSceneSequence(canvas, text, style, date){
  ...
  function frame(now){
    ...
    drawProps(ctx,W,H,GY,S,autoProps,ss.chars,style,ss.localT,scene.location);
